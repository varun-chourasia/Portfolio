    ...
    reveals.forEach((element) => {
      ...
      if (elementTop < windowHeight - elementVisible) {
        ...
        if (element.classList.contains("skill-item")) {
          const skillItems =
            element.parentElement.querySelectorAll(".skill-item");
...
function initIntersectionObserver() {
  ...
  document
    .querySelectorAll(
      '[class*="fade-"], [class*="scale-"], .skill-category, .project-card, .cert-card'
    )
    .forEach((el) => {
      observer.observe(el);
    });
...
const skillSelect = document.querySelector(".skills-select");
const skillCategories = document.querySelectorAll(".skill-category");
...
if (skillSelect) {
  skillSelect.addEventListener("change", () => {
    ...
    skillCategories.forEach((category) => {
      if (category.dataset.category === selected) {
  ...
  // show first category by default on load
